...
def checkUpdate():
    ...
    try:
        r1 = requests.get("https://gitee.com/hosiang1026/DailyRemind/raw/master/scripts/qinglong/py/ql_gold_task.py").text
        r2 = re.findall(re.compile("version = \d.\d"), r1)[0].split("=")[1].strip()
        if float(r2) > version:
            print("发现新版本：" + r2)
            print("正在自动更新脚本...")
            os.system("ql raw https://gitee.com/hosiang1026/DailyRemind/raw/master/scripts/qinglong/py/ql_gold_task.py &")
    except:
        pass
...
if __name__ == '__main__':
    ...
    if load_send():
        ...
        if isinstance(newcontent, str) and newcontent.strip():
            ...
            print('获取黄金价格失败！', newcontent if not isinstance(newcontent, str) else '')
